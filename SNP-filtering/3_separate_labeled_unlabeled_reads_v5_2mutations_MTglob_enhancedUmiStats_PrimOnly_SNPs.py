...
if len(sys.argv) == 8:
    ...
    print("Please provide me with an input .bam file to extract labeled reads from, a minimum quality score for mutations, the number of required mutations for a UMI as well as a file with SNP positions, the cutoff mutation rate for a SNP [%] and the minumum coverage. Additionally please let me know if the file uses cellranger [CR] or STAR/Rsubread [ST] gene tags. CB/UB tags are expected for cell barcode and UMI.")
    sys.exit()
